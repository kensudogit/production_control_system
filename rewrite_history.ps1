# PowerShell script to rewrite git history and remove API key
# コミット履歴を書き換えてAPIキーを削除するスクリプト

$ErrorActionPreference = "Stop"

Write-Host "==========================================" -ForegroundColor Cyan
Write-Host "コミット履歴を書き換えてAPIキーを削除" -ForegroundColor Cyan
Write-Host "==========================================" -ForegroundColor Cyan
Write-Host ""
...
if (Test-Path $targetFile) {
    git add $targetFile
    git commit -m "fix: Add corrected DemandForecasting.tsx without hardcoded API key" --no-verify
